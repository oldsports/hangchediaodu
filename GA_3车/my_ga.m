...
for generation = 1 : maximal_generation % 演化循环开始
    ...
    while(1)
        if generation==1
            population=Copy_of_select_car(3,20,D,ta,population_size);%改任务数
        end
        ...
        if generation~=1||best_fitness(generation,1)<20000
            break
        end
    end
    ...
    mutation_population(mutation_points) = ceil(3*rand(1, number_of_mutations)); % 对选中的基因进行变异操作
    %3是车数，其他问题需要改
