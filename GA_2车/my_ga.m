...
% 最佳适应度
% 每一代的最佳适应度都先初始化为0
best_fitness = zeros(maximal_generation, 7);
...
for generation = 1 : maximal_generation % 演化循环开始
    ...
    while(1)
        if generation==1
            population=Copy_of_select_car(2,number_of_variables,D,ta,population_size);%改车数
        end
        ...
        % index记录排序后每个值原来的行数
        [cost, index] = sortrows(cost); % 将适应度函数值从小到大排序
        ...
        %达到预设的最高适应度后跳出，防止最初的适应度过高
        if generation~=1||best_fitness(generation,1)<4500
            break
        end
    end
    generation
    ...
    % 若本代的最优解已足够好，则停止演化
    if best_fitness(generation,1) < minimal_cost; 
    ...
    mutation_population(mutation_points) = ceil(2*rand(1, number_of_mutations)); % 对选中的基因进行变异操作
    %2是车数，其他问题需要改
